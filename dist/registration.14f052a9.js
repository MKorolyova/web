...
function generateDays(month, year) {
    ...
    let monthMarkup = ``;
    for(let i = firstDay; i > 0; i--)monthMarkup += `<div class="date not_active_date">${pevMonthLastDate - i + 1}</div>`;
    ...
    for(let i = lastDay; i < 6; i++)monthMarkup += `<div class="date not_active_date">${i - lastDay + 1}</div>`;
    return monthMarkup;
}
...
const customerInfo = {};
function chooseDate(event1) {
    const day = event1.target;
    ...
    document.getElementById("time").addEventListener("click", ()=>{
        ...
        if (hour.classList.contains("hour")) {
            ...
            hour.parentElement.querySelectorAll(".hour").forEach((h)=>{
                if (h !== hour) h.classList.remove("selected");
            });
...
document.addEventListener("DOMContentLoaded", ()=>{
    document.getElementById("registration_frame").addEventListener("click", (event1)=>{
        const pricingPlan = event1.target;
        if (pricingPlan.classList.contains("pricing_plan")) {
            ...
            pricingPlan.parentElement.querySelectorAll(".pricing_plan").forEach((p)=>{
                if (p !== pricingPlan) p.classList.remove("background_light");
            });
            const plan = pricingPlan.querySelector(".subtitle")?.textContent;
            if (plan == "Drop-in Class") document.getElementById("drop_in_class_registration").classList.remove("invisible");
            else document.getElementById("drop_in_class_registration").classList.add("invisible");
            customerInfo.pricingPlan = plan;
        }
    });
});
function sendForm(event1) {
...
fetch(url2).then((response)=>{
    ...
    console.log(`\u{422}\u{435}\u{43C}\u{43F}\u{435}\u{440}\u{430}\u{442}\u{443}\u{440}\u{430}: ${temperature.toFixed(1)}\xb0C`);
    console.log(`\u{41F}\u{43E}\u{433}\u{43E}\u{434}\u{430}: ${weatherDescription}`);
    document.getElementById("weather_forecast").innerHTML = ` ${temperature.toFixed(0)}\xb0C ` + weatherDescription;
});

//# sourceMappingURL=registration.14f052a9.js.map
